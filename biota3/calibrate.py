...
def extractGamma0(dataloc, year, shp, plot_field, agb_field, buffer_size = 0, verbose = False, units = 'natural'):
    ...
    plot_names = biota3.mask.getField(shp, plot_field)

    if agb_field is not None:
        agb = biota3.mask.getField(shp, agb_field).astype(np.float)

    # Identify tiles that contain gamma0 data for the shapefile
    tiles = biota3.mask.getTilesInShapefile(shp)
    ...
    for lat, lon in tiles:
        ...
        # Load tile
        try:
            tile = biota3.LoadTile(dataloc, lat, lon, year, downsample_factor = downsample_factor, lee_filter = lee_filter, window_size = window_size)
        except:
            continue
        ...
        # Mask out each plot
        plot_mask = biota3.mask.maskShapefile(tile, shp, location_id = True, buffer_size = buffer_size)
