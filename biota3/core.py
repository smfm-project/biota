...
import biota3.filter
import biota3.indices
import biota3.IO
import biota3.mask
import biota3.SM
...
class LoadTile(object):
    ...
    def __getSize(self, filepath):
        ...
        y_size, x_size = biota3.IO.loadSize(filepath)
    ...
    def __getProj(self):
        ...
        proj = biota3.IO.loadProjection(self.__getHHPath())
    ...
    def __getMask(self, masked_px_count = False, output = False, show = False):
        ...
        mask = biota3.IO.loadArray(self.mask_path) != 255
    ...
    def updateMask(self, filename, buffer_size = 0., classes = [], output = False, show = False):
        ...
        # Load mask
        mask = biota3.mask.updateMask(self, filename, buffer_size = buffer_size, classes = classes)
    ...
    def getDN(self, polarisation = 'HV', output = False, show = False):
        ...
        if polarisation == 'HV':
            DN = biota3.IO.loadArray(self.HV_path)
        else:
            DN = biota3.IO.loadArray(self.HH_path)
    ...
    def __getDay(self):
        ...
        day_after_launch = biota3.IO.loadArray(self.date_path)
    ...
    def getSM(self, output = False, show = False, search_days = 7):
        ...
        if not hasattr(self, 'SM'):
            ...
            SM = biota3.SM.getSM(self, search_days = search_days, interpolation = self.SM_interpolation)
    ...
    def getWoodyCover(self, output = False, show = False):
        ...
        if not hasattr(self, 'WoodyCover'):
            ...
            if self.area_threshold > 0:
                ...
                contiguous_area, _ = biota3.indices.getContiguousAreas(WoodyCover, True, min_pixels = min_pixels, contiguity = self.contiguity)
    ...
    def getForestPatches(self, output = False, show = False):
        ...
        if not hasattr(self, 'ForestPatches'):
            ...
            _, ForestPatches = biota3.indices.getContiguousAreas(WoodyCover, True, min_pixels = min_pixels, contiguity = self.contiguity)
    ...
    def __outputGeoTiff(self, data, output_name, dtype = 6):
        ...
        # Write to disk
        biota3.IO.outputGeoTiff(data, filename, self.geo_t, self.proj, output_dir = self.output_dir, dtype = dtype, nodata = nodata)
    ...
    def __showArray(self, data, title = '', cbartitle = '', vmin = None, vmax = None, cmap = None):
        ...
        biota3.IO.showFigure(data, self.lat, self.lon, title = title, cbartitle = cbartitle, vmin = vmin, vmax = vmax, cmap = cmap)
    ...
    def getChangeType(self, output = False, show = False):
        ...
        if not hasattr(self, 'ChangeType'):
            ...
            if self.change_area_threshold > 0:
                ...
                CHANGE_INCREASE, _ = biota3.indices.getContiguousAreas(CHANGE & INCREASE & (NF_F | F_F), True, min_pixels = min_pixels, contiguity = self.contiguity)
                CHANGE_DECREASE, _ = biota3.indices.getContiguousAreas(CHANGE & DECREASE & (F_NF | F_F), True, min_pixels = min_pixels, contiguity = self.contiguity)
